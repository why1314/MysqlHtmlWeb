    ...
    <body>
        ...
        <div class="wrap">
            <div class="bread_box">
                ...
                <span> &gt;</span>
                <a href="<%=request.getContextPath()%>/route?method=findRouteByPage&cid=${route.category.cid}&PageNumber=1">${route.category.cname}</a><span> &gt;</span>
                <a href="#">${route.rname}</a>
            </div>
            <div class="prosum_box">
                <dl class="prosum_left">
                    <dt>
                        <img id="big_img" alt="" class="big_img" src="${route.rimage}">
                    </dt>
                    <dd>
                        ...
                        <c:forEach items="${route.routeImgList}" var="routeImg" varStatus="status">
                            ...
                            <c:if test="${status.count>4}">
                                <a style="display: none" title="" class="little_img" data-bigpic="${routeImg.bigPic}">
                                    <img src="${routeImg.smallPic}">
                                </a>
                            </c:if>
                        </c:forEach>
                ...
                <div class="prosum_right">
                    ...
                    <div class="pros_price">
                        ...
                        <p class="collect">
                            <a id="okBtn" href="JavaScript:void(0);" onclick="addMyFavorite()" class="btn">
                                <i class="glyphicon glyphicon-heart-empty"></i>点击收藏
                            </a>

                            <a id="noBtn" class="btn already" disabled="disabled">
                                <i class="glyphicon glyphicon-heart-empty"></i>点击收藏
                            </a>
                            <span>已收藏：${route.count}</span>
                        </p>
                    </div>
                </div>
            </div>
            <script>
                /* 页面加载成功后，发送ajax请求，查询用户是否登录，根据用户状态显示按钮状态*/
                $(function () {
                    var url = "<%=request.getContextPath()%>/favorite";
                    var params = {"rid":${route.rid}, "method": "isFavorite"};
            ...
            <script>
                function addMyFavorite() {
                    var url = "<%=request.getContextPath()%>/favorite";
                    var params = {"method": "addMyFavorite", "rid":${route.rid}};
